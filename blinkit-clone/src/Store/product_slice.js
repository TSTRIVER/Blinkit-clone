...
export const createProduct =
  (name, price, quantity, image) => async (dispatch) => {
    try {
      const { data } = await api.post(`/api/v1/createProduct`, {
        name,
        price,
        quantity,
        image,
...
export const updateProduct =
  (id, name, price, quantity, image) => async (dispatch) => {
    try {
      const { data } = await api.put(`/api/v1/updateprod/${id}`, {
        name,
        price,
        quantity,
        image,
...
export const deleteProduct = (id) => async (dispatch) => {
  try {
    await api.delete(`/api/v1/delprod/${id}`);
