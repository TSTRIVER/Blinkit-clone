...
export const getYourAddress =
  (userid, setas, receiver, flat, street) => async (dispatch) => {
    try {
      await api.post(`/api/v1/createAddress`, {
        userid,
        setas,
        receiver,
        flat,
...
export const getAddresses = (userid) => async (dispatch) => {
  try {
    const { data } = await api.get(`/api/v1/getAddresses/${userid}`);
...
export const updateAddress =
  (selected_ind, userid, setas, receiver, flat, street) => async (dispatch) => {
    try {
      await api.put(`/api/v1/updateAddress/${userid}`, {
        selected_ind,
        setas,
        receiver,
        flat,
